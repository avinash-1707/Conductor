...
// Dev-only default — refused outright in production (superRefine; Unit 28a).
...
const envSchema = z
  .object({
    ...
    // Liveness/readiness HTTP surface for platform health checks (Unit 28a).
  ...
  .superRefine((value, ctx) => {
    // Production must never run on dev secrets or the canned model (Unit 28a).
    ...
    if (value.LLM_MODE === "mock") {
      ctx.addIssue({
        code: "custom",
        message: "LLM_MODE=mock is an E2E/dev facility — never run it in production",
        path: ["LLM_MODE"],
      });
    }
  });
